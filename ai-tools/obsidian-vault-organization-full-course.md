...
## 附录：知识库 Obsidian 文档生态索引

本知识库已积累多篇 Obsidian 相关指南，各有侧重。下图帮你快速定位应该读哪篇：

| 文档 | 作者 | 侧重点 | 适合谁 |
|------|------|--------|--------|
| 📄 **本篇** (vault 组织) | CyrilXBT | 检索优先的文件夹/标签/属性/MOC 架构 | 已有笔记但找不快的人 |
| 📄 [个人操作系统](../ai-tools/obsidian/obsidian-personal-operating-system.md) | CyrilXBT | AI 自动化三层架构（Claude+N8N），8 文件夹版 | 想用 AI 自动维护 vault 的人 |
| 📄 [小白入门教程](../ai-tools/obsidian/obsidian-beginner-complete-tutorial.md) | 黄小木 | 安装/Markdown/插件推荐/模板 | 纯新手（从零开始） |
| 📄 [业务自动运行 vault](../ai-tools/obsidian/obsidian-business-ops-vault-with-claude-n8n.md) | CyrilXBT | 商业场景的 vault 自动化 | 自由职业/创业者 |
| 📄 [仪表盘构建](../ai-tools/obsidian/obsidian-dashboard-everything-that-matters-today.md) | CyrilXBT | Dataview 仪表盘 + 每日概览 | 每天需要一页全览的人 |
| 📄 [LLM Wiki 完全指南](../ai-tools/obsidian/obsidian-llm-wiki-complete-guide.md) | — | Karpathy 风格的 LLM 驱动知识系统 | 技术背景、喜欢 AI-native 的人 |

### 同一作者（CyrilXBT）的两次文件夹结构对比

CyrilXBT 有两篇关于 vault 组织架构的文章，分别对应**纯个人知识管理版**（本篇）和 **AI 自动化增强版**（个人操作系统），两者是演进关系：

| 本篇（纯组织版） | 个人操作系统（AI 版） | 说明 |
|-----------------|----------------------|------|
| 00 - INBOX | 00 - CAPTURE | 功能相同，都是捕获暂存 |
| 01 - NOTES（含 daily/meetings/books/courses） | 01 - ACTIVE（含 projects/areas/daily） | NOTES 更宽泛；ACTIVE 只保留当前活内容 |
| — | 04 - GENERATED | AI 版新增：Claude 输出存放区，**不要手动编辑** |
| — | 05 - QUEUE | AI 版新增：Claude 的待处理任务收件箱 |
| — | 06 - CALENDAR | AI 版新增：基于时间的信息跟踪 |
| 02 - PROJECTS | 含于 01 - ACTIVE/projects/ | 项目结构相同 |
| 03 - AREAS | 含于 01 - ACTIVE/areas/ | 领域管理相同 |
| 04 - RESOURCES | 02 - RESOURCES | 参考材料，功能一致 |
| 05 - ARCHIVE | 07 - ARCHIVE | 归档，功能一致 |
| 06 - SYSTEM | 03 - SYSTEM | 基础设施，功能一致 |

**什么时候用哪个？**
- **纯组织版（本篇）**：如果你的 Obsidian 主要是手动使用，偶尔用 Dataview 查询——7 文件夹就够了
- **AI 版**：如果你跑 Claude Code + Filesystem MCP + N8N 自动化——8 文件夹版多了 Generated/Queue/Calendar 三个 AI 工作流专用层

**好消息：** 两套结构可以无损共存。AI 版是在纯组织版的基础上加了三个新文件夹（04/05/06），没有破坏原有结构。你可以从本篇开始，等上了 AI 自动化再扩展。

---

### 本知识库 Obsidian 文档之间的编排关系

```
Obsidian 知识体系
│
├─ 🌱 新手入门
│   └─ 小白入门教程（安装、Markdown、核心概念、插件）
│
├─ 🏗️ 组织架构（核心）
│   ├─ 本篇：检索优先的 vault 组织（文件夹/标签/属性/MOC）
│   └─ LLM Wiki 指南（AI-native 的替代组织思路）
│
├─ 🤖 自动化增强
│   ├─ 个人操作系统（Claude + N8N 三层架构）
│   └─ 业务自动运行 vault（商业场景变体）
│
└─ 📊 日常使用
    └─ 仪表盘构建（Dataview + 每日概览）
...
| 你的阶段 | 按顺序读 |
|---------|---------|
| **第一次用 Obsidian** | 小白入门教程 → 本篇（组织方法论）→ 仪表盘构建 |
| **已有笔记但混乱** | 本篇（从现在开始渐进重构）→ 个人操作系统（如果想上 AI） |
| **已有系统但想升级** | 本篇（对照检查架构）→ 个人操作系统（三层架构）→ LLM Wiki（另一思路） |
| **做商业/项目** | 本篇（基础组织）→ 业务自动运行 vault → 个人操作系统 |

---

*Processed on 2026-05-27 from https://x.com/cyrilxbt/status/2058373087330959829*

*交叉索引更新于 2026-05-27 — 与知识库内 [个人操作系统](../ai-tools/obsidian/obsidian-personal-operating-system.md)、[小白入门教程](../ai-tools/obsidian/obsidian-beginner-complete-tutorial.md)、[仪表盘](../ai-tools/obsidian/obsidian-dashboard-everything-that-matters-today.md)、[LLM Wiki](../ai-tools/obsidian/obsidian-llm-wiki-complete-guide.md)、[业务自动化 vault](../ai-tools/obsidian/obsidian-business-ops-vault-with-claude-n8n.md) 对齐*
