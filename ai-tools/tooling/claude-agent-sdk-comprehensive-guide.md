...
| 参数 | 说明 |
|------|------|
| `allowed_tools` / `allowedTools` | 工具白名单（字符串数组） |
...
|------|------|
| `cwd` | 工作目录 |
| `setting_sources` / `settingSources` | 从项目配置文件加载设置 |
...
1. **Skills 注入**：在 `AgentDefinition.skills` 中指定，内容直接注入子代理
