...
- **[10-claude-code-agents-pipeline.md](../agent-engineering/patterns/10-claude-code-agents-pipeline.md)** — 本文的 Skill 关键事实（修改不会自动写回）解释了为什么 Agent Pipeline 文章强调手动配置 Skill 文件而非对话修改。
- **[npx-skills-add-flow.md](../agent-engineering/harness/npx-skills-add-flow.md)** — 七、八节（Skill 编辑 + 自动化）与 npx skills add 的安装流程对应：Skill 需要手动编辑文件，自动化可以通过 `/schedule` 实现。
