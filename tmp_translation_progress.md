...
本章涵盖大型语言模型的基础架构以及使训练和推理高效的关键优化技术。主题按课程顺序排列：我们从 Transformer 本身开始，然后介绍如何高效训练、如何低成本适配、如何压缩、如何扩展以及如何加速推理。

---

## 1.1 LLM 工作原理：直观概述

在深入架构细节之前，让我们建立对大型语言模型如何将文本转化为文本的直觉。整个过程遵循一个简单的流水线：

**文本 → 令牌 → 表征 → 令牌 → 文本**

### 四个关键阶段

1. **分词化（Tokenization）**：原始文本被拆分为子词片段（不是字符，也不是完整单词），使用学习得到的词汇表。"unhappiness" 可能变成 ["un", "happiness"] 或 ["unhapp", "iness"]。

2. **嵌入（Embedding）**：每个令牌 ID 索引到一个学习到的嵌入表中，产生一个 ℝᵈ 中的稠密向量（通常 d=4096）。这些向量捕捉语义含义——相似的词获得相似的向量。

3. **上下文处理（Contextual Processing）**：Transformer 堆栈并行处理所有嵌入，利用自注意力机制让每个位置可以从所有其他位置"读取"信息。经过 L 层后，每个位置的隐藏状态编码了丰富的上下文信息。

4. **预测（Prediction）**：最终的隐藏状态被投影到整个词汇表上的概率分布，再由解码策略选择下一个令牌。

---

## 1.2 分词化（Tokenization）

分词化是将原始文本转换为语言模型操作的离散符号的关键第一步。分词器的选择直接影响模型质量、多语言能力和计算效率。

**为什么用子词？** 字符级模型需要非常长的序列（昂贵的注意力计算）。词级模型无法处理罕见或新颖的词汇。子词分词达到了理想的平衡：常见词是单个令牌（"the" → [the]），罕见词分解为已知片段（"cryptocurrency" → ["crypt", "ocur", "rency"]），词汇表也保持可控规模（32K–128K 个令牌）。

### 1.2.1 为什么不用字符或词？

| 粒度 | 词汇表大小 | 序列长度 | 问题 |
|------|-----------|---------|------|
| 字符 | ~256 | 非常长 | 注意力 O(n²) 成本；难以学习长程语义 |
| 词语 | ~500K+ | 短 | 无法处理罕见/新词；嵌入表巨大 |
| 子词 | 32K–128K | 适中 | 最佳权衡：短序列，开放词汇 |

### 1.2.2 字节对编码（BPE）

BPE 是 GPT、Llama、Mistral 和大多数现代 LLM 使用的主流分词算法。

**BPE 算法：**
1. 从单个字符（字节）的词汇表开始
2. 统计训练语料中所有相邻符号对的频率
3. 将最频繁的对合并为一个新符号
4. 重复步骤 2–3 进行 k 次迭代（直到达到目标词汇表大小）

### 1.2.3 其他分词方法

| 方法 | 使用方 | 核心思路 |
|------|-------|---------|
| BPE | GPT-4, Llama-3, Mistral | 自底向上合并频繁对；确定性算法 |
| WordPiece | BERT, DistilBERT | 类似于 BPE，但最大化训练数据的似然 |
| Unigram LM | SentencePiece (T5, XLNet) | 自顶向下：从大词汇表开始，按似然影响剪枝 |
| Byte-level BPE | GPT-2+ | 在原始字节上运行 BPE（无未知令牌）；256 基础词汇 |

### 1.2.4 分词最佳实践

1. **词汇表大小很重要**：32K 是最低限度；128K 能更好覆盖多语言和代码。Llama-3 使用 128K 令牌。
2. **特殊令牌**：始终包含 `<bos>`、`<eos>`、`<pad>`、`<unk>`。对于指令微调模型，添加角色标记（`<|user|>`、`<|assistant|>`）。
3. **多令牌率（Fertility）**：跨语言测量每词的令牌数。高值表示该语言覆盖不佳。
4. **绝不跨边界分词**：空格、标点和数字应一致处理。
5. **数字**：对于算术任务，考虑数字级分词。"2024" 作为 ["2","0","2","4"] 实现逐位推理。
6. **代码**：确保空白（缩进）被高效分词。Llama-3 将连续空格序列作为单个令牌。

### 1.2.5 特殊令牌与结构化提示

特殊令牌是为结构含义而非语言内容保留的词汇表条目，对控制模型行为至关重要。

**角色标记的最佳实践：**
- 绝不拆分特殊令牌——它们必须是原子的
- SFT 期间在特殊令牌上掩蔽损失，模型不应"学习"预测格式
- 通过特殊令牌而非自然语言指令编码任务语义
- RL 中参考模型和策略模型必须使用相同的分词和特殊令牌处理
- EOS 处理：确保 EOS 包含在动作空间中，否则响应会无限增长

---

## 1.3 Transformer 架构

Transformer 是所有现代 LLM 的基础。理解其组件对于掌握本指南中的每一种优化和训练方法至关重要。

### 1.3.1 高层结构

仅解码器（decoder-only）Transformer 通过嵌入层、重复的注意力+FFN 块和最终的词汇表 logits 投影来顺序处理令牌。每个子层（注意力、FFN）之前都有 LayerNorm，之后有残差连接：x + SubLayer(LN(x))。这种 Pre-Norm 排序（Llama、GPT-3、Mistral 使用）无需预热即可稳定训练，与原始的 Post-Norm 相反。

### 1.3.2 原始编码器-解码器 Transformer

Transformer 最初被提出时是一种**编码器-解码器**架构，用于序列到序列任务（机器翻译、摘要）。虽然现代 LLM 主要使用仅解码器变体（GPT 风格），但理解完整架构仍然重要。

**编码器**：双向处理整个输入序列——每个令牌关注所有其他令牌。**解码器**：使用因果掩码自回归生成令牌，并包含交叉注意力层来"读取"编码器输出。

现代 LLM（GPT、Llama、Qwen）仅使用解码器部分，去掉了编码器和交叉注意力层。

### 1.3.3 仅解码器 vs 编码器-解码器

仅解码器模型更简单（一个模型、一个损失），扩展性更好（所有参数都为生成做贡献），支持统一训练（预训练=下一令牌预测=微调目标）。编码器-解码器在纯生成任务上浪费了编码器容量。

### 1.3.4 嵌入：从离散令牌到连续空间

嵌入是离散符号的学习稠密向量表征。核心洞见：**相似的概念获得相近的向量**。向量算术可以捕捉关系：king − man + woman ≈ queen。

**嵌入表**：本质是一个矩阵 E ∈ ℝ^(|V|×d)。对于 Llama-3：128,256 × 4,096 = 5.25 亿参数（8B 模型的 6.5%）。

**各向异性问题**：预训练嵌入往往是高度**各向异性**的——它们占据嵌入空间中的一个狭窄锥体，导致 RAG/检索中所有嵌入的余弦相似度都偏高。

**解决方法——白化（Whitening）**：一种线性变换 h̃ = D^(-1/2) U^T (h − μ)，使嵌入分布各向同性，余弦相似度恢复意义。

### 1.3.5 自注意力机制

自注意力允许每个令牌关注序列中的其他每个令牌，基于相关性计算加权组合。

**缩放点积注意力**：Attention(Q, K, V) = softmax(QK^T/√d_k + M)V

**计算复杂度**：朴素注意力在序列长度上具有**二次方成本**——O(n²·d) 的时间和 O(n²) 的内存。对于 128K 上下文，注意力矩阵单是 FP32 就有 64 GB。

**驯服注意力成本的方法：**
1. **FlashAttention**（IO 感知）：消除在 HBM 中物化 n×n 矩阵的需求
2. **滑动窗口/局部注意力**：每个令牌只关注最近 w 个令牌，成本 O(n·w)
3. **稀疏注意力模式**：局部窗口 + 周期性全局令牌
4. **线性注意力/状态空间模型**：Mamba、RWKV，理论上 O(n·d²)
5. **KV 缓存压缩**：推理时压缩或驱逐旧的 KV 对

**注意**：FlashAttention 不是稀疏注意力的替代品——它是在底层运行注意力核的 IO 优化，可以与任何注意力掩码自由组合。

### 1.3.6 多头注意力

并行运行多个注意力操作，每个学习关注输入的不同方面。

**分组查询注意力（GQA）**：Llama-3 使用更少的 KV 头（如 8 个）共享给更多 Q 头（如 32 个），KV 缓存大小减少 4 倍，质量损失极小。

### 1.3.7 位置编码

Transformer 本质上是置换等变的——没有位置信息，模型无法区分"猫坐在垫子上"和"垫子坐猫在上"。

| 方法 | 使用方 | 核心思路 |
|------|-------|---------|
| 正弦波 | 原始 Transformer | 固定 sin/cos 不同频率 |
| 学习绝对位置 | GPT-2, BERT | 每个位置的学习嵌入，受限于训练长度 |
| RoPE（旋转） | Llama, Qwen, Mistral | 按位置角度旋转 Q、K 向量 |
| ALiBi | BLOOM, MPT | 无位置嵌入，添加线性偏置到注意力分数 |

RoPE 是当前主导方案。通过 NTK 感知缩放和 YaRN 等方法可以扩展到超长上下文（100K–1M+ 令牌）。

### 1.3.8 前馈网络（MLP）

每个 Transformer 块包含一个独立应用于每个位置的 MLP。现代 LLM 使用 **SwiGLU** 激活函数。研究表明 FFN 层充当**键值存储器**——W₁ 行是键（要匹配的模式），W₂ 列是值（要输出的信息）。

### 1.3.9 层归一化（Layer Normalization）

层归一化通过对特征维度的激活进行归一化来稳定训练。**Pre-LN** 是现代 LLM 的标准（Pre-Norm：x + SubLayer(LN(x))）。**RMSNorm**（Llama、Mistral 使用）简化版 LayerNorm，去掉均值居中，速度快 5–10%，质量相当。

### 1.3.10 模型规模参考

| 模型 | 参数 | 层数 | d | 注意力头 | KV 头 | 上下文 |
|------|------|------|------|---------|-------|-------|
| Llama-3.1 8B | 8B | 32 | 4096 | 32 | 8 | 128K |
| Llama-3.1 405B | 405B | 126 | 16384 | 128 | 8 | 128K |
| DeepSeek-V3 | 671B(37B活跃) | 61 | 7168 | 128 | MLA | 128K |

### 1.3.11 注意力病理现象

**注意力陷阱（Attention Sink）**：Transformer 模型向序列中的第一个令牌分配不成比例的高注意力分数。这是因为 softmax 必须产生有效的概率分布，当没有键与查询特别相关时，模型需要一个"倾倒"位置。解决方案：StreamingLLM（保留前 k 个令牌）和专用陷阱令牌。

**注意力稀释（Attention Dilution）**：序列长度增长时，每个查询的注意力预算被分散到更多键上，导致"中间迷失"（Lost in the Middle）效应——位于长上下文中间的信息被忽略。

其他现象：注意力头专门化、归纳头（induction heads）、注意力崩塌、检索头等。
